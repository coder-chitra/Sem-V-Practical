...
void calculateTimes(struct Process processes[], int n, double *avgTurnaroundTime, double *avgWaitingTime) {
    ...
    *avgWaitingTime = 0;
    ...
    while (remaining > 0) {
        ...
        for (int i = 0; i < n; i++) {
            if (processes[i].arrivalTime <= clock && processes[i].cpuBurst > 0 && processes[i].cpuBurst < minBurst) {
        ...
        if (index != -1) {
            // Update clock and process times
            clock += processes[index].cpuBurst;
            ...
            *avgWaitingTime += processes[index].waitingTime;
            processes[index].cpuBurst = 0; // Mark process as completed
            remaining--;
        } else {
            clock++; // Increment clock if no process is available
        }
    }
